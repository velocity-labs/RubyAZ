class BlogPost < ActiveRecord::Base
  ...
  scope :non_drafts, lambda { where( "draft = ?", false) }
  ...
  class << self
    ...
    # events
    def events_by_month(month = nil, year = nil)
      today = Date.today
      month ||= today.month
      year ||= today.year
      beginning_of_month = Date.new(year, month, 1) 
      end_of_month = beginning_of_month.end_of_month
      BlogCategory.where(:title => "Events").first.posts.where(:published_at => (beginning_of_month..end_of_month)).group_by { |post| post.published_at.strftime("%d") }      
    end
